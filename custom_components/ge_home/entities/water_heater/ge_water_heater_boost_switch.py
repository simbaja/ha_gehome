...
class GeWaterHeaterBoostSwitch(GeErdSwitch):
    ...
    def __init__(self, api: ApplianceApi):
        ...
        super().__init__(api, ErdCode.WH_WATER_HEATER_BOOST_MODE, ErdOnOffBoolConverter())
    ...
    def is_on(self) -> bool:
        ...
        # Use the boost state to determine if the boost mode is on
        boost_state = self.appliance.get_erd_value(ErdCode.WH_WATER_HEATER_BOOST_STATE)
    ...
    async def async_turn_on(self, **kwargs):
        ...
        await self.appliance.async_set_erd_value(ErdCode.WH_WATER_HEATER_BOOST_MODE, ErdOnOff.ON)
    
    async def async_turn_off(self, **kwargs):
        """Turn the boost mode off."""
        _LOGGER.debug(f"Turning off water heater boost mode {self.unique_id}")
        await self.appliance.async_set_erd_value(ErdCode.WH_WATER_HEATER_BOOST_MODE, ErdOnOff.OFF)
