...
class GeHomeUpdateCoordinator(DataUpdateCoordinator):
    ...
    def _reset_initialization(self):
        self.client: Optional[GeWebsocketClient] = None
    ...
    def create_ge_client(
        ...
        """Create a new GeClient object with callbacks wired up."""
    ...
    def online(self) -> bool:
        """True if connected, or within retry count tolerance."""
        return self.connected or self._retry_count <= RETRY_OFFLINE_COUNT

    @property
    def connected(self) -> bool:
        """True if the GE client is connected."""
    ...
    def _get_appliance_api(self, appliance: GeAppliance) -> ApplianceApi:
        ...
        _LOGGER.debug(
            f"Creating API {api_type.__name__} for appliance {appliance.mac_addr} ({appliance.appliance_type})"
        )
    ...
    async def async_begin_session(self):
        ...
        fut = asyncio.ensure_future(
            self.client.async_run_client(), loop=self.hass.loop
        )
    ...
    async def _kill_client(self):
        """Kill the client. Debug/testing only."""
    ...
    async def async_reconnect(self) -> None:
        ...
        try:
            ...
            _LOGGER.warning(
                f"Could not reconnect: {err}, will retry in {self._get_retry_delay()}s"
            )
    ...
    async def on_connect(self, _):
        ...
        _LOGGER.debug("Connected to GE Home service")
        self.last_update_success = True
        self._retry_count = 0

    async def async_maybe_trigger_all_ready(self):
        """See if we're all ready to go, and if so, let HA know."""
        if self._init_done:
            return
